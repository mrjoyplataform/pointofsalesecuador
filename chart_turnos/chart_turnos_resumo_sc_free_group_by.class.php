...
class chart_turnos_resumo
{
   ...
   var $sum_idempleado_fondo_inicial;
   ...
   var $sum_idempleado_total_pagos;
   var $sc_proc_quebra_idturno;
   var $count_idturno;
   var $sum_idturno_fondo_inicial;
   var $sum_idturno_diferencia;
   var $sum_idturno_entrega_efectivo;
   var $sum_idturno_entrega_tarjeta;
   var $sum_idturno_sistema_efectivo;
   var $sum_idturno_sistema_tarjeta;
   var $sum_idturno_sistema_total;
   var $sum_idturno_total_pagos;
   var $sc_proc_quebra_fecha_cierre;
   var $count_fecha_cierre;
   var $sum_fecha_cierre_fondo_inicial;
   ...
   var $sum_fecha_cierre_total_pagos;
   var $sc_proc_quebra_estado;
   var $count_estado;
   var $sum_estado_fondo_inicial;
   ...
   function __construct($tipo = "")
   {
      ...
      $this->array_total_idturno      = array();
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['under_dashboard'] ? "off" : "on";

      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_turnos'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_turnos'];
      ...
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['chart_turnos']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['chart_turnos']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['chart_turnos']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_step']     = 'image';
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_step'] = 'end';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['chart_turnos']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['campos_busca'];
         ...
         {
             ob_start();
             $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_type'] = $_POST['fc_chart_type'];
         }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys']))
       {
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_x_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order'] = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type']  = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values']     = array();

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_type']        = 'column2d';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_rule']  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_field'] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_rule_groupby']["idempleado"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_field_groupby']["idempleado"] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_rule_groupby']["idturno"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_field_groupby']["idturno"] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_rule_groupby']["fecha_cierre"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_field_groupby']["fecha_cierre"] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_rule_groupby']["estado"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_combination_order_field_groupby']["estado"] = 'label';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys'][0] = 0;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys'][1] = 1;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys'][2] = 2;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys'][3] = 3;
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][6] = '';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][7] = '';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][8] = '';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][9] = '';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][2] = '';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][3] = '';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][4] = '';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_field_display_type'][5] = '';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][6] = null;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][7] = null;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][8] = null;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][9] = null;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][2] = null;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][3] = null;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][4] = null;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_display_values'][5] = null;


           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by'][] = 6;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by'][] = 7;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by'][] = 8;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by'][] = 9;
       }

      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pesq_tab_label']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pesq_tab_label'] = "";
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_search_add']))
      {
          $this->monta_css();
          $nmgp_tab_label = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pesq_tab_label'];
          ...
          $Seq_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_search_add']['seq'];
          $Cmp_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_search_add']['cmp'];
          ...
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_search_add']);
          exit;
      }
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['res_hrz'];
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']) && !$this->NM_export)
      {
          foreach ($_SESSION['scriptcase']['sc_aba_iframe'] as $aba => $apls_aba)
          {
              if (in_array("chart_turnos", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][4];
      $this->array_total_geral[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][5];
      $this->array_total_geral[2] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][6];
      $this->array_total_geral[3] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][7];
      $this->array_total_geral[4] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][2];
      $this->array_total_geral[5] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][8];
      $this->array_total_geral[6] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][9];
      $this->array_total_geral[7] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][3];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['tot_geral'][1] == 0)
      {
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_total']['idempleado'] = $this->array_total_idempleado;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_total']['idturno'] = $this->array_total_idturno;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_total']['fecha_cierre'] = $this->array_total_fecha_cierre;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_total']['estado'] = $this->array_total_estado;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['json_use_label'])
       {
       ...
       $Lab_fecha_cierre =  sprintf("" . $this->Ini->Nm_lang['lang_othr_chart_title_HHII'] . " fecha_cierre", $Lab_fecha_cierre);
       $Lab_estado = "Estado";
       }
       $prep_field = array();
       $prep_field['idempleado'] = $Lab_idempleado;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idempleado'] = $Lab_idempleado; 
       $prep_field['idturno'] = $Lab_idturno;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idturno'] = $Lab_idturno; 
       $prep_field['fecha_cierre'] = $Lab_fecha_cierre;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['fecha_cierre'] = $Lab_fecha_cierre; 
       $prep_field['estado'] = $Lab_estado;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['estado'] = $Lab_estado; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idturno']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idturno'] = "Idturno"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idempleado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idempleado'] = "Idempleado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idcaja']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['idcaja'] = "Idcaja"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['fecha_apertura']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['fecha_apertura'] = "Fecha Apertura"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['fondo_inicial']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['fondo_inicial'] = "Fondo Inicial"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['estado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['estado'] = "Estado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['fecha_cierre']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['fecha_cierre'] = "Fecha Cierre"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['diferencia']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['diferencia'] = "Diferencia"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['total_entrega']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['total_entrega'] = "Total Entrega"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['numero_trans']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['numero_trans'] = "Numero Trans"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['entrega_efectivo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['entrega_efectivo'] = "Entrega Efectivo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['entrega_tarjeta']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['entrega_tarjeta'] = "Entrega Tarjeta"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['sistema_efectivo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['sistema_efectivo'] = "Sistema Efectivo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['sistema_tarjeta']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['sistema_tarjeta'] = "Sistema Tarjeta"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['sistema_total']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['sistema_total'] = "Sistema Total"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['total_pagos']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['labels']['total_pagos'] = "Total Pagos"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       //-----
       $this->comp_sum = array(
           2 => "Entrega Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
           3 => "Entrega Tarjeta (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
           4 => "Sistema Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
           5 => "Sistema Tarjeta (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
           6 => "Fondo Inicial (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
           7 => "Sistema Total (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
           8 => "Total Entrega",
           9 => "Diferencia (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
       );
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
       ...
       //-----
       $this->comp_sum_lnk = array(
           2 => array('field' => "entrega_efectivo", 'show' => true),
           3 => array('field' => "entrega_tarjeta", 'show' => true),
           4 => array('field' => "sistema_efectivo", 'show' => true),
           5 => array('field' => "sistema_tarjeta", 'show' => true),
           6 => array('field' => "fondo_inicial", 'show' => true),
           7 => array('field' => "sistema_total", 'show' => true),
           8 => array('field' => "total_pagos", 'show' => true),
           9 => array('field' => "diferencia", 'show' => true),
       );

       //-----
       $this->comp_sum_css = array(
           2 => "css_entrega_efectivo_sum",
           3 => "css_entrega_tarjeta_sum",
           4 => "css_sistema_efectivo_sum",
           5 => "css_sistema_tarjeta_sum",
           6 => "css_fondo_inicial_sum",
           7 => "css_sistema_total_sum",
           8 => "css_total_pagos_sum",
           9 => "css_diferencia_sum",
       );

       //-----
       $this->comp_sum_nm = array(
           2 => "entrega_efectivo_sum",
           3 => "entrega_tarjeta_sum",
           4 => "sistema_efectivo_sum",
           5 => "sistema_tarjeta_sum",
           6 => "fondo_inicial_sum",
           7 => "sistema_total_sum",
           8 => "total_pagos_sum",
           9 => "diferencia_sum",
      ...
      for ($ix = 0; $ix < sizeof($Arr_gb); $ix++)
      {
          ...
          $Str_gb .= "         \$this->comp_index[" . $ix . "][\$dados_" . $Arr_gb[$ix] . "[10] ] = \$dados_" . $Arr_gb[$ix] . "[9];";
          $Str_gb .= "         \$this->comp_label[" . $ix . "]" . $Arr_lab[$ix] . "[ \$dados_" . $Arr_gb[$ix] . "[10] ] = \$dados_" . $Arr_gb[$ix] . "[9];";
          $Str_gb .= "    }";
      }
      for ($ix = 0; $ix < sizeof($Arr_gb); $ix++)
      {
      ...
      eval ($Str_gb);
       ...
       $Arr_parms['idturno']['alin'] = "''";
       $Arr_parms['idturno']['link'] = "S";
       ...
       $Arr_parms['idturno']['fill'] = "true";
       $Arr_parms['idturno']['order'] = "value";
       $Arr_parms['idturno']['order_start'] = 'asc';
       $Arr_parms['idturno']['order_invert'] = 'false';
       $Arr_parms['idturno']['order_enabled'] = 'true';
       $Arr_parms['idturno']['order_datatype'] = 'int';
       ...
       $prep_links_fl['idturno'] = array(0 => 'idturno', 1 => '');
       $prep_links_fl['fecha_cierre'] = array(0 => 'fecha_cierre', 1 => '@aspass@');
       $prep_links_fl['estado'] = array(0 => 'estado', 1 => '');
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_turnos@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['proc_pdf_vert'])
   {
     $this->comp_tab_hover = true;
   }
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           'idempleado' => array(
           '1' => array(
               'title'    => $this->getChartText(strip_tags("Entrega Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Entrega Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(1),
           ),
           '2' => array(
               'title'    => $this->getChartText(strip_tags("Entrega Tarjeta (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[3],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Entrega Tarjeta (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(2),
           ),
           '3' => array(
               'title'    => $this->getChartText(strip_tags("Sistema Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[4],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Sistema Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(3),
           ),
           '4' => array(
               'title'    => $this->getChartText(strip_tags("Sistema Tarjeta (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[5],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Sistema Tarjeta (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(4),
           ),
           '5' => array(
               'title'    => $this->getChartText(strip_tags("Fondo Inicial (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[6],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Fondo Inicial (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(5),
           ),
           '6' => array(
               'title'    => $this->getChartText(strip_tags("Sistema Total (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[7],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Sistema Total (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(6),
           ),
           '7' => array(
               'title'    => $this->getChartText(strip_tags("Total Entrega")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[8],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Total Entrega")),
               'format'   => $this->formatChartValue(7),
           ),
           '8' => array(
               'title'    => $this->getChartText(strip_tags("Diferencia (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'idempleado',
               'field_y'  => $this->comp_sum_nm[9],
               'label_x'  => $this->getChartText(strip_tags($Lab_idempleado)),
               'label_y'  => $this->getChartText(strip_tags("Diferencia (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
           ...
           '1' => array(
               ...
               'field_x'  => 'idturno',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_idturno)),
               'label_y'  => $this->getChartText(strip_tags("Entrega Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(1),
           ),
           '2' => array(
           ...
           '1' => array(
               ...
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'fecha_cierre',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_fecha_cierre)),
               'label_y'  => $this->getChartText(strip_tags("Entrega Efectivo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(1),
           ),
           '2' => array(
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_order']['sc_free_group_by'] as $i_summ_order)
       {
                   ...
                   foreach ($this->sc_comb_quick_index[$i_index_summ] as $i_index_comb_groupby)
                   {
                       ...
                       if (!isset($a_chart_combinations[$s_comb_index]))
                       {
                           $a_chart_combinations[$s_comb_index] = array(
                               ...
                               'label_x'     => '',
                               'label_y'     => '',
                               'xml_pholder' => sizeof($this->comp_y_axys) > $i_index_group + 1 ? 'sc_chart_turnos_' . session_id() . '_!!!C_' . ($i_index_group + 1) . '_' . $s_index_comb . ('' == $s_index_comb ? '' : '_') . '__SC_PLACEHOLDER__!!!.json' : '',
                               'xml'         => 'sc_chart_turnos_' . session_id() . '_!!!C_' . $i_index_group . '_' . $s_index_comb . '!!!.json',
                           );
                       }
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'];
	}
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
	...
	function filterChartsByMultiseries() {
		...
		if ($applyFilter && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_drill_down']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['summarizing_drill_down'])) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'] as $chartIndex => $chartData) {
				$indexParts = explode('|', $chartIndex);
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_chart_turnos_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_chart_turnos_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['embutida'])
                   {
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
      {
      ...
      { 
         ...
         { 
             $seq_search = 1;
             foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq'] as $cmp => $def)
             {
                $Seq_grid      = $seq_search;
                $Cmp_grid      = $cmp;
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_turnos']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_turnos']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_turnos']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_turnos']) . "_";
               } 
           }
       }
   ...
   function resumo_final()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['ajax_nav'])
      {
   ...
   {
      $this->Tot_ger = false;
      require_once($this->Ini->path_aplicacao . "chart_turnos_total.class.php"); 
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      $this->Total   = new chart_turnos_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_refresh_after_chart'] = 'resumo';
      $nm_saida->saida("<HTML" . $_SESSION['scriptcase']['reg_conf']['html_dir'] . ">\r\n");
      $nm_saida->saida("<HEAD>\r\n");
      $nm_saida->saida(" <TITLE>" . $this->Ini->Nm_lang['lang_othr_chart_title'] . " turnos</TITLE>\r\n");
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_turnos_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_turnos_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_turnos_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "chart_turnos/chart_turnos_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" href=\"" . $this->Ini->path_prod . "/third/jquery_plugin/thickbox/thickbox.css\" type=\"text/css\" media=\"screen\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
      } 
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_turnos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_chart_turnos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_turnos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css'] . '.css');
     ...
     if (!$this->Ini->Export_html_zip)     {
      ...
      $nm_saida->saida(" </style>\r\n");
           ...
           $nm_saida->saida("     }\r\n");
           $nm_saida->saida("   </script>\r\n");
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['ajax_nav'] && !$this->Ini->sc_export_ajax && !$this->Ini->Export_html_zip && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['doc_word'])
      {
           $nm_saida->saida("   <script type=\"text/javascript\"> \r\n");
          ...
          if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq']))
          { 
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['remove_margin'] ? 'margin: 0;' : '';
          $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['remove_border'] ? 'border-width: 0;' : '';
          $vertical_center = '';
          $nm_saida->saida(" <BODY id=\"grid_chart\" class=\"" . $this->css_scGridPage . "\" style=\"" . $remove_margin . $vertical_center . "\">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['embutida'])
      {
...
;
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_pdf.php?nm_opc=pdf&nm_target=0&nm_cor=cor&papel=8&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=s&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&pdf_zip=N&origem=chart&language=es&conf_socor=N&is_chart_app=Y&nm_label_group=N&nm_all_cab=S&nm_all_label=S&nm_orient_grid=2&script_case_init=" . $this->Ini->sc_page . "&app_name=chart_turnos&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['word'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bword", "", "", "Rword_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_word.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_cor=CO&nm_res_cons=s&nm_ini_word_res=chart&nm_all_modules=chart&password=n&origem=chart&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['xml'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bxml", "", "", "Rxml_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_xml.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&password=n&nm_res_cons=s&nm_ini_xml_res=chart&nm_all_modules=chart&nm_xml_tag=tag&nm_xml_label=S&language=&origem=chart&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['csv'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bcsv", "", "", "Rcsv_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_csv.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_res_cons=s&nm_ini_csv_res=chart&nm_all_modules=chart&password=n&nm_delim_line=1&nm_delim_col=1&nm_delim_dados=1&nm_label_csv=N&language=&origem=chart&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Rprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_print.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_opc=resumo&nm_cor=CO&nm_res_cons=s&nm_ini_prt_res=chart&nm_all_modules=chart&password=n&origem=chart&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['group_2'] == "on")
      {
         ...
         if (is_file("chart_turnos_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("chart_turnos_help.txt"); 
   ...
   function monta_barra_top_mobile()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['word'] == "on" && !$this->NM_res_sem_reg)
      {
          ...
          $nm_saida->saida("            <div id=\"div_Rword_top\" class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $this->nm_btn_exist['word'][] = "Rword_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bword", "", "", "Rword_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_word.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_cor=CO&nm_res_cons=s&nm_ini_word_res=chart&nm_all_modules=chart&password=n&origem=chart&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['xls'] == "on" && !$this->NM_res_sem_reg)
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['xml'] == "on" && !$this->NM_res_sem_reg)
      {
          ...
          $nm_saida->saida("            <div id=\"div_Rxml_top\" class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $this->nm_btn_exist['xml'][] = "Rxml_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bxml", "", "", "Rxml_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_xml.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&password=n&nm_res_cons=s&nm_ini_xml_res=chart&nm_all_modules=chart&nm_xml_tag=tag&nm_xml_label=S&language=&origem=chart&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['csv'] == "on" && !$this->NM_res_sem_reg)
      {
          ...
          $nm_saida->saida("            <div id=\"div_Rcsv_top\" class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $this->nm_btn_exist['csv'][] = "Rcsv_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bcsv", "", "", "Rcsv_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_csv.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_res_cons=s&nm_ini_csv_res=chart&nm_all_modules=chart&password=n&nm_delim_line=1&nm_delim_col=1&nm_delim_dados=1&nm_label_csv=N&language=&origem=chart&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['rtf'] == "on" && !$this->NM_res_sem_reg)
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
          ...
          $nm_saida->saida("            <div id=\"div_Rprinttop\" class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $this->nm_btn_exist['print'][] = "Rprint_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Rprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_turnos/chart_turnos_config_print.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_opc=resumo&nm_cor=CO&nm_res_cons=s&nm_ini_prt_res=chart&nm_all_modules=chart&password=n&origem=chart&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_3", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
   ...
   function monta_html_fim()
   {
      ...
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_turnos']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_turnos']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
$nm_saida->saida("       scJs_alert('" . $mensagem . "', $jsonParams);\r\n");
      ...
      elseif ($this->Ini->Export_html_zip || ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['opcao'] == "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['graf_pdf'] == "S"))
      {
          $this->grafico_pdf();
      }
      else
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'chart_turnos_config_pivot.php?nome_apl=chart_turnos&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"chart_turnos_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   }\r\n");
          ...
          $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "chart_turnos/chart_turnos_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\"+ str_type +\"&sAdd=__E__nmgp_cor_word=\" + cor + \"__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
          $nm_saida->saida("       }\r\n");
          $nm_saida->saida("       else\r\n");
          $nm_saida->saida("       {\r\n");
          ...
          $nm_saida->saida("               document.Fprint.action = \"chart_turnos_export_ctrl.php\";\r\n");
          $nm_saida->saida("           }\r\n");
          $nm_saida->saida("           else\r\n");
          $nm_saida->saida("           {\r\n");
          ...
          $nm_saida->saida("       document.Fexport.action = \"chart_turnos_export_ctrl.php\";\r\n");
      ...
      if (!extension_loaded("zip"))
      {
          ...
          $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
          $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_turnos_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_turnos@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
...
;
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_turnos_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "chart_turnos/chart_turnos_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['skip_charts']))
   {
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'] as $chart_index => $chart_data)
       {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               chart_turnos_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['this_chart_label'] = '';
           $this->Graf->monta_grafico($chart_index, 'pdf');
           if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['this_chart_label']) {
               $nm_saida->saida("<script type=\"text/javascript\">$(\"#$random_id\").html(\"{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['this_chart_label']}\")</script>\r\n");
           }
       ...
       if ($bChartFP)
       {
           ...
           chart_turnos_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_list']))
   {
       $prim_graf = true;
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['chart_list'] as $arr_chart)
       {
           if (!$prim_graf)
           {
               $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
...
//--- 
 function grafico_bot()
 {
   ...
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['graf_bot'] = true;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['graf_first'] = true;
       require_once($this->Ini->path_aplicacao . $this->Ini->Apl_grafico); 
       $this->Graf  = new chart_turnos_grafico();
       $this->Graf->Db     = $this->Db;
       $this->Graf->Erro   = $this->Erro;
       ...
       $nm_saida->saida("<TR>\r\n");
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts']);
       $sChartLang  = isset($this->Ini->Nm_lang['lang_pdff_pcht']) ? $this->Ini->Nm_lang['lang_pdff_pcht'] : 'Generating chart';
       if (!NM_is_utf8($sChartLang))
       {
           $sChartLang = sc_convert_encoding($sChartLang, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      if ($bPDF && (!isset($this->progress_fp) || !$this->progress_fp))
      {
           ...
           chart_turnos_pdf_progress_call("PDF\n", $this->Ini->Nm_lang);
           chart_turnos_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_turnos_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_turnos_pdf_progress_call("100\n", $this->Ini->Nm_lang);
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['pivot_charts'] as $chart_index => $chart_data)
       {
           ...
           if ($bPDF)
           {
               ...
               if ($this->progress_fp)
               {
                   chart_turnos_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "/" . $iChartTotal . "...\n", $this->Ini->Nm_lang, true);
       ...
       $nm_saida->saida("</TD>\r\n");
       $nm_saida->saida("</TR>\r\n");
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['graf_bot'] = false;
       if ($bPDF && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['charts_html'])
       {
           $nm_saida->saida("<script type=\"text/javascript\">\r\n");
           $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['charts_html']}\r\n");
           $nm_saida->saida("</script>\r\n");
...
//--- 
   ...
   {
      global $nm_saida;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['cond_pesq']))
      {  
          $pos       = 0;
          $trab_pos  = false;
          $pos_tmp   = true; 
          $tmp       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['cond_pesq'];
          while ($pos_tmp)
          {
             $pos = strpos($tmp, "##*@@", $pos);
             if ($pos !== false)
          ...
          $nm_cond_filtro_or  = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['cond_pesq'], $trab_pos + 5) == "or")  ? " " . trim($this->Ini->Nm_lang['lang_srch_orr_cond']) . " " : "";
          $nm_cond_filtro_and = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['cond_pesq'], $trab_pos + 5) == "and") ? " " . trim($this->Ini->Nm_lang['lang_srch_and_cond']) . " " : "";
          $nm_cab_filtro   = substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['cond_pesq'], 0, $trab_pos);
   ...
   //---- 
   function adiciona_registro($sum_fondo_inicial, $sum_diferencia, $sum_entrega_efectivo, $sum_entrega_tarjeta, $sum_sistema_efectivo, $sum_sistema_tarjeta, $sum_sistema_total, $sum_total_pagos, $quebra_idempleado, $quebra_idturno, $quebra_fecha_cierre, $quebra_estado, $quebra_idempleado_orig, $quebra_idturno_orig, $quebra_fecha_cierre_orig, $quebra_estado_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
          ...
          $cmp_look   = "Cmp_formt";
          $cmp_orig   = "Cmp_arg";
          if (empty($sum_fondo_inicial)) {$sum_fondo_inicial = '0';}
          ...
          if (empty($sum_total_pagos)) {$sum_total_pagos = '0';}
          eval ('
          if (!isset($this->' . $arr_name . '))
          {
              $this->' . $arr_name . '[0] = 1;
              $this->' . $arr_name . '[1] = ' . $sum_fondo_inicial . ';
              $this->' . $arr_name . '[2] = ' . $sum_diferencia . ';
              $this->' . $arr_name . '[3] = ' . $sum_entrega_efectivo . ';
              $this->' . $arr_name . '[4] = ' . $sum_entrega_tarjeta . ';
              $this->' . $arr_name . '[5] = ' . $sum_sistema_efectivo . ';
              $this->' . $arr_name . '[6] = ' . $sum_sistema_tarjeta . ';
              $this->' . $arr_name . '[7] = ' . $sum_sistema_total . ';
              $this->' . $arr_name . '[8] = ' . $sum_total_pagos . ';
              $this->' . $arr_name . '[9] = "' . addslashes($$cmp_look) . '";
              $this->' . $arr_name . '[10] = "' . $$cmp_orig . '";
          }
          else
          {
              $this->' . $arr_name . '[0]++;
              $this->' . $arr_name . '[1] = bcadd($this->' . $arr_name . '[1], ' . $sum_fondo_inicial . ', 10);
              $this->' . $arr_name . '[2] = bcadd($this->' . $arr_name . '[2], ' . $sum_diferencia . ', 10);
              $this->' . $arr_name . '[3] = bcadd($this->' . $arr_name . '[3], ' . $sum_entrega_efectivo . ', 10);
              $this->' . $arr_name . '[4] = bcadd($this->' . $arr_name . '[4], ' . $sum_entrega_tarjeta . ', 10);
              $this->' . $arr_name . '[5] = bcadd($this->' . $arr_name . '[5], ' . $sum_sistema_efectivo . ', 10);
              $this->' . $arr_name . '[6] = bcadd($this->' . $arr_name . '[6], ' . $sum_sistema_tarjeta . ', 10);
              $this->' . $arr_name . '[7] = bcadd($this->' . $arr_name . '[7], ' . $sum_sistema_total . ', 10);
              $this->' . $arr_name . '[8] = bcadd($this->' . $arr_name . '[8], ' . $sum_total_pagos . ', 10);
          }
          ');
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     { 
         $nmgp_select = "SELECT idturno, idempleado, idcaja, str_replace (convert(char(10),fecha_apertura,102), '.', '-') + ' ' + convert(char(8),fecha_apertura,20), fondo_inicial, estado, str_replace (convert(char(10),fecha_cierre,102), '.', '-') + ' ' + convert(char(8),fecha_cierre,20), diferencia, total_entrega, numero_trans, entrega_efectivo, entrega_tarjeta, sistema_efectivo, sistema_tarjeta, sistema_total, total_pagos from " . $this->Ini->nm_tabela; 
     } 
     elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mysql))
     { 
         $nmgp_select = "SELECT idturno, idempleado, idcaja, fecha_apertura, fondo_inicial, estado, fecha_cierre, diferencia, total_entrega, numero_trans, entrega_efectivo, entrega_tarjeta, sistema_efectivo, sistema_tarjeta, sistema_total, total_pagos from " . $this->Ini->nm_tabela; 
     } 
     elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
     { 
         $nmgp_select = "SELECT idturno, idempleado, idcaja, convert(char(23),fecha_apertura,121), fondo_inicial, estado, convert(char(23),fecha_cierre,121), diferencia, total_entrega, numero_trans, entrega_efectivo, entrega_tarjeta, sistema_efectivo, sistema_tarjeta, sistema_total, total_pagos from " . $this->Ini->nm_tabela; 
     } 
     else 
     { 
         $nmgp_select = "SELECT idturno, idempleado, idcaja, fecha_apertura, fondo_inicial, estado, fecha_cierre, diferencia, total_entrega, numero_trans, entrega_efectivo, entrega_tarjeta, sistema_efectivo, sistema_tarjeta, sistema_total, total_pagos from " . $this->Ini->nm_tabela; 
     } 
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['where_pesq']; 
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->fecha_apertura = $rs_res->fields[3] ;  
            $this->fondo_inicial = $rs_res->fields[4] ;  
            $this->fondo_inicial =  str_replace(",", ".", $this->fondo_inicial);
            $this->estado = $rs_res->fields[5] ;  
            $this->fecha_cierre = $rs_res->fields[6] ;  
            $this->diferencia = $rs_res->fields[7] ;  
            $this->diferencia =  str_replace(",", ".", $this->diferencia);
            $this->total_entrega = $rs_res->fields[8] ;  
            $this->total_entrega =  str_replace(",", ".", $this->total_entrega);
            $this->numero_trans = $rs_res->fields[9] ;  
            $this->entrega_efectivo = $rs_res->fields[10] ;  
            $this->entrega_efectivo =  str_replace(",", ".", $this->entrega_efectivo);
            $this->entrega_tarjeta = $rs_res->fields[11] ;  
            $this->entrega_tarjeta =  str_replace(",", ".", $this->entrega_tarjeta);
            $this->sistema_efectivo = $rs_res->fields[12] ;  
            $this->sistema_efectivo =  str_replace(",", ".", $this->sistema_efectivo);
            $this->sistema_tarjeta = $rs_res->fields[13] ;  
            $this->sistema_tarjeta =  str_replace(",", ".", $this->sistema_tarjeta);
            $this->sistema_total = $rs_res->fields[14] ;  
            $this->sistema_total =  str_replace(",", ".", $this->sistema_total);
            $this->total_pagos = $rs_res->fields[15] ;  
            $this->total_pagos =  str_replace(",", ".", $this->total_pagos);
            $this->idempleado_orig = $this->idempleado;
            $this->idturno_orig = $this->idturno;
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->fondo_inicial, $this->diferencia, $this->entrega_efectivo, $this->entrega_tarjeta, $this->sistema_efectivo, $this->sistema_tarjeta, $this->sistema_total, $this->total_pagos, $this->idempleado, $this->idturno, $this->fecha_cierre, $this->estado, $this->idempleado_orig,  $this->idturno_orig,  $this->fecha_cierre_orig,  $this->estado_orig);
            }
   }
//
   ...
   function html_grid_search()
   {
       ...
       $str_display = empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq'])?'none':'';
       if(!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['ajax_nav'])
       {
          $nm_saida->saida("   <tr id=\"NM_Grid_Search\" ajax='' style='display:" . $str_display . "'>\r\n");
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['ajax_nav'] && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq']))
       { 
           $_SESSION['scriptcase']['saida_html'] = "";
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq'] as $cmp => $def)
           {
           ...
           if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['save_grid']))
           {
               return;
           }
           else
       ...
       $nm_saida->saida("            <input type=\"hidden\" name=\"parm\" value=\"\"/> \r\n");
            if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq']))
            {
                foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq'] as $cmp => $def)
                {
                    if (isset($def['label'])) {
     ...
     { 
       ...
       { 
           ...
           $_SESSION['scriptcase']['saida_html'] = "";
           if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['save_grid']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq']))
           { 
   ...
   function grid_search_add_tag()
   {
       ...
       if (count($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq']) < 0)
       {
           $lin_obj .= "<table id='id_grid_search_all_cmp' cellpadding=0 cellspacing=0>";
           foreach ($All_cmp_search as $cada_cmp)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_turnos']['grid_pesq'][$cada_cmp]))
               {
                   $lin_obj .= "  <tr>";
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "pointofsales/chart_turnos";
