  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_cantidad_tokens_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_cantidad_tokens_mob_form();
	}
...
?>
  ...
  // ---------- Validate titulo
  function do_ajax_control_cantidad_tokens_mob_validate_titulo()
  {
    var nomeCampo_titulo = "titulo";
    var var_titulo = scAjaxGetFieldHidden(nomeCampo_titulo);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cantidad_tokens_mob_validate_titulo(var_titulo, var_script_case_init, do_ajax_control_cantidad_tokens_mob_validate_titulo_cb);
  } // do_ajax_control_cantidad_tokens_mob_validate_titulo

  function do_ajax_control_cantidad_tokens_mob_validate_titulo_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cantidad_tokens_mob_validate_titulo_cb

  // ---------- Validate cantidad
  function do_ajax_control_cantidad_tokens_mob_validate_cantidad()
  {
    var nomeCampo_cantidad = "cantidad";
    var var_cantidad = scAjaxGetFieldText(nomeCampo_cantidad);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cantidad_tokens_mob_validate_cantidad(var_cantidad, var_script_case_init, do_ajax_control_cantidad_tokens_mob_validate_cantidad_cb);
  } // do_ajax_control_cantidad_tokens_mob_validate_cantidad

  function do_ajax_control_cantidad_tokens_mob_validate_cantidad_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cantidad_tokens_mob_validate_cantidad_cb

  // ---------- Validate total
  function do_ajax_control_cantidad_tokens_mob_validate_total()
  {
    var nomeCampo_total = "total";
    var var_total = scAjaxGetFieldHidden(nomeCampo_total);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cantidad_tokens_mob_validate_total(var_total, var_script_case_init, do_ajax_control_cantidad_tokens_mob_validate_total_cb);
  } // do_ajax_control_cantidad_tokens_mob_validate_total

  function do_ajax_control_cantidad_tokens_mob_validate_total_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "total";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cantidad_tokens_mob_validate_total_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_cantidad_tokens_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_cantidad_tokens_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn();
    x_ajax_control_cantidad_tokens_mob_submit_form(var_titulo, var_cantidad, var_total, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, do_ajax_control_cantidad_tokens_mob_submit_form_cb);
  } // do_ajax_control_cantidad_tokens_mob_submit_form

  function do_ajax_control_cantidad_tokens_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_cantidad_tokens_mob_submit_form_cb_after_alert);
  } // do_ajax_control_cantidad_tokens_mob_submit_form_cb
  function do_ajax_control_cantidad_tokens_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_cantidad_tokens_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_cantidad_tokens_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_control_cantidad_tokens_mob_navigate_form, function() {});
    } else {
      perform_ajax_control_cantidad_tokens_mob_navigate_form();
    }
  }

  function perform_ajax_control_cantidad_tokens_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_cantidad_tokens_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_cantidad_tokens_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_cantidad_tokens_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_cantidad_tokens_mob_navigate_form_cb_after_alert);
  } // do_ajax_control_cantidad_tokens_mob_navigate_form_cb
  function do_ajax_control_cantidad_tokens_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_control_cantidad_tokens_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_cantidad_tokens_mob_navigate_form_cb_after_alert

  function sc_hide_control_cantidad_tokens_mob_form()
  {
  ...
  var ajax_error_list = {
    ...
    "cantidad": {"label": "CANTIDAD TOKENS", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
