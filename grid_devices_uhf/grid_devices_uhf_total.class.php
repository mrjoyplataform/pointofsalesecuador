...
class grid_devices_uhf_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_devices_uhf']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_devices_uhf']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_iddevice_dataid_tag($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $nm_comando = "select distinct      fecha,     dataid,     iddevice,          tag,     estado from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['where_pesq']; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['tot_geral'] = array() ;  
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['contr_total_geral'] = "OK";
   } 

   //-----  iddevice
   function quebra_iddevice_iddevice_dataid_tag($iddevice, $arg_sum_iddevice) 
   {
      global $tot_iddevice;
      $tot_iddevice = array() ;  
      $tot_iddevice[0] = $iddevice ; 
   }
   //-----  dataid
   function quebra_dataid_iddevice_dataid_tag($iddevice, $dataid, $arg_sum_iddevice, $arg_sum_dataid) 
   {
      global $tot_dataid;
      $tot_dataid = array() ;  
      $tot_dataid[0] = $dataid ; 
   ...
   //----- 
   function Calc_resumo_iddevice_dataid_tag($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('iddevice' => "iddevice",'dataid' => "dataid");
      $cmps_quebra_atual = array("iddevice","dataid");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_iddevice_dataid_tag($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_iddevice_dataid_tag($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('iddevice' => 'N','estado' => 'N');
      $nm_comando = "select distinct      fecha,     dataid,     iddevice,          tag,     estado from " . $this->Ini->nm_tabela . " " . $this->sc_where_atual; 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*)#@#cmps_quebras#@# from (" . $nm_comando . ") sc_sel_esp " . $where_ok;
         $comando  = "select count(*)#@#cmps_quebras#@# from (";
         $comando .= "select " . $cmps_quebras1 . " from (" . $nm_comando . ") sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_uhf']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['iddevice']['reg'] = "S";
      $format_dimensions['iddevice']['msk'] = "";
      $format_dimensions['dataid']['reg'] = "S";
      $format_dimensions['dataid']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'iddevice_dataid_tag', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "iddevice") {
          ...
          { 
              ...
              $$Cada_dim = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("HH", ""));
          } 
              }
              if (null === $$Cada_dim)
              {
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_iddevice_dataid_tag($where_ok);
