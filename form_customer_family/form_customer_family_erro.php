...
$old_error_handler = set_error_handler("form_customer_family_scriptcase_error_handler");

function form_customer_family_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_customer_family']['contr_erro']) && $_SESSION['scriptcase']['form_customer_family']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_customer_family->contr_form_customer_family->Apl_com_erro = true;
            if ($inicial_form_customer_family->contr_form_customer_family->NM_ajax_flag)
            {
                $inicial_form_customer_family->contr_form_customer_family->NM_ajax_info['errList']['geral_form_customer_family'][] = $msg;
                return;
            }
            form_customer_family_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_customer_family_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_customer_family'])  && '' != $_SESSION['scriptcase']['error_icon']['form_customer_family'])  ? $_SESSION['scriptcase']['error_icon']['form_customer_family']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_customer_family']) && '' != $_SESSION['scriptcase']['error_close']['form_customer_family']) ? $_SESSION['scriptcase']['error_close']['form_customer_family'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_customer_family']) && '' != $_SESSION['scriptcase']['error_span_title']['form_customer_family']) ? $_SESSION['scriptcase']['error_span_title']['form_customer_family'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_customer_family']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_customer_family']) ? $_SESSION['scriptcase']['error_icon_title']['form_customer_family'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_customer_family']['error_buffer'] .= ob_get_flush();
}
