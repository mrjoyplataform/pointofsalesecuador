...
class grid_movimtorno_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_movimtorno']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_movimtorno']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $cod_device;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_cod_device_fecha($res_limit=false)
   {
      global $nada, $nm_lang , $cod_device;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['contr_total_geral'] == "OK") 
      { 
   ...
   function quebra_fecha_sc_free_group_by($fecha, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $cod_device;
      $tot_fecha = array() ;  
      $tot_fecha[0] = $fecha ; 
   }
   //-----  cod_device
   function quebra_cod_device_sc_free_group_by($cod_device, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $cod_device;
      $tot_cod_device = array() ;  
      $tot_cod_device[0] = $cod_device ; 
   }
   //-----  usuario
   ...
   //-----  cod_device
   function quebra_cod_device_cod_device_fecha($cod_device, $arg_sum_cod_device) 
   {
      global $tot_cod_device, $cod_device;
      $tot_cod_device = array() ;  
      $tot_cod_device[0] = $cod_device ; 
   }
   //-----  fecha
   function quebra_fecha_cod_device_fecha($cod_device, $fecha, $arg_sum_cod_device, $arg_sum_fecha) 
   {
      global $tot_fecha, $cod_device;
      $tot_fecha = array() ;  
      $tot_fecha[0] = $fecha ; 
   }
   //-----  usuario
   function quebra_usuario_cod_device_fecha($cod_device, $fecha, $usuario, $arg_sum_cod_device, $arg_sum_fecha, $arg_sum_usuario) 
   {
      global $tot_usuario, $cod_device;
      $tot_usuario = array() ;  
      $tot_usuario[0] = $usuario ; 
   }
   ...
   {
      global $nm_lang, $cod_device;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fecha' => "fecha",'cod_device' => "cod_device",'usuario' => "usuario");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('idreg' => 'N','cod_device' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['cod_device']['reg'] = "S";
      $format_dimensions['cod_device']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "cod_device") {
                  $this->Lookup->lookup_sc_free_group_by_cod_device($$Cada_dim,  $cod_device);
              }
              if ($Cada_dim == "usuario") {
              }
              if (null === $$Cada_dim)
   ...
   //----- 
   function Calc_resumo_cod_device_fecha($destino_resumo)
   {
      global $nm_lang, $cod_device;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('cod_device' => "cod_device",'fecha' => "fecha",'usuario' => "usuario");
      $cmps_quebra_atual = array("cod_device","fecha","usuario");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_cod_device_fecha($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_movimtorno']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_cod_device_fecha($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'cod_device_fecha', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "cod_device") {
                  $this->Lookup->lookup_cod_device_fecha_cod_device($$Cada_dim,  $cod_device);
              }
              if ($Cada_dim == "usuario") {
              }
              if (null === $$Cada_dim)
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_cod_device_fecha($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
