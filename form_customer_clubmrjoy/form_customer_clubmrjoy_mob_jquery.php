...
function scFocusField(sField) {
  ...
  if ($oField.length > 0) {
    switch ($oField[0].name) {
      ...
      case 'celular':
      case 'cliente_tipo':
        sc_exib_ocult_pag('form_customer_clubmrjoy_mob_form0');
        break;
      case 'provincia':
      case 'canton':
      case 'parroquia':
      case 'direccion':
      case 'sector':
        sc_exib_ocult_pag('form_customer_clubmrjoy_mob_form1');
        break;
      case 'family':
        sc_exib_ocult_pag('form_customer_clubmrjoy_mob_form2');
        break;
    }
  }
...
function displayChange_field_family(row, status) {
    var fieldId;
	if ("on" == status && typeof $("#nmsc_iframe_liga_form_customer_family_mob")[0].contentWindow.scRecreateSelect2 === "function") {
		$("#nmsc_iframe_liga_form_customer_family_mob")[0].contentWindow.scRecreateSelect2();
	}
}
...
function scHidePage(pageNo) {
	$("#id_form_customer_clubmrjoy_mob_form" + pageNo).hide();
}
...
function scJQCalendarAdd(iSeqRow) {
  $("#id_sc_field_ult_fecha" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_customer_clubmrjoy_mob_validate_ult_fecha(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
...
?>
  ...
  $("#id_sc_field_afiliado_desde" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_customer_clubmrjoy_mob_validate_afiliado_desde(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_customer_clubmrjoy_mob')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
