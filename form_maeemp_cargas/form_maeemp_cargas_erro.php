...
$old_error_handler = set_error_handler("form_maeemp_cargas_scriptcase_error_handler");

function form_maeemp_cargas_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_maeemp_cargas']['contr_erro']) && $_SESSION['scriptcase']['form_maeemp_cargas']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_maeemp_cargas->contr_form_maeemp_cargas->Apl_com_erro = true;
            if ($inicial_form_maeemp_cargas->contr_form_maeemp_cargas->NM_ajax_flag)
            {
                $inicial_form_maeemp_cargas->contr_form_maeemp_cargas->NM_ajax_info['errList']['geral_form_maeemp_cargas'][] = $msg;
                return;
            }
            form_maeemp_cargas_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_maeemp_cargas_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_maeemp_cargas'])  && '' != $_SESSION['scriptcase']['error_icon']['form_maeemp_cargas'])  ? $_SESSION['scriptcase']['error_icon']['form_maeemp_cargas']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_maeemp_cargas']) && '' != $_SESSION['scriptcase']['error_close']['form_maeemp_cargas']) ? $_SESSION['scriptcase']['error_close']['form_maeemp_cargas'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_maeemp_cargas']) && '' != $_SESSION['scriptcase']['error_span_title']['form_maeemp_cargas']) ? $_SESSION['scriptcase']['error_span_title']['form_maeemp_cargas'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_maeemp_cargas']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_maeemp_cargas']) ? $_SESSION['scriptcase']['error_icon_title']['form_maeemp_cargas'] : '';
?>
