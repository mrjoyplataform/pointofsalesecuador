...
class grid_sales_detail_temp_resumen_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_sales_detail_temp_resumen']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_sales_detail_temp_resumen']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_id_product($res_limit=false)
   {
      global $nada, $nm_lang , $id_product;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(quantity), sum(item_total) from (SELECT     id_sales_detail,     id_sales,     unit_cost,     unit_value,     quantity,     discount,     item_total,     id_product,     idcaja,     uniqueid, (select image from product where id_product=sales_detail_temp.id_product) as imagen , uniqueidtrans, (select product_name from product where id_product=sales_detail_temp.id_product) as product_name    FROM     sales_detail_temp where estado=0 and idcaja=" . $_SESSION['idcaja'] . " ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(quantity), sum(item_total) from (SELECT     id_sales_detail,     id_sales,     unit_cost,     unit_value,     quantity,     discount,     item_total,     id_product,     idcaja,     uniqueid, (select image from product where id_product=sales_detail_temp.id_product) as imagen , uniqueidtrans, (select product_name from product where id_product=sales_detail_temp.id_product) as product_name    FROM     sales_detail_temp where estado=0 and idcaja=" . $_SESSION['idcaja'] . " ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['contr_total_geral'] = "OK";
   } 

   //-----  id_product
   function quebra_id_product_id_product($id_product, $arg_sum_id_product) 
   {
      ...
      { 
         $nm_comando .= " where id_product" . $arg_sum_id_product ; 
      } 
      else 
      { 
         $nm_comando .= " and id_product" . $arg_sum_id_product ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //----- 
   function Calc_resumo_id_product($destino_resumo)
   {
      global $nm_lang, $quitar, $codigoqr, $id_product;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('id_product' => "id_product");
      $cmps_quebra_atual = array("id_product");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_id_product($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_id_product($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('id_sales_detail' => 'N','id_sales' => 'N','unit_cost' => 'N','unit_value' => 'N','quantity' => 'N','discount' => 'N','item_total' => 'N','id_product' => 'N','idcaja' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(quantity), sum(item_total)#@#cmps_quebras#@# from (SELECT     id_sales_detail,     id_sales,     unit_cost,     unit_value,     quantity,     discount,     item_total,     id_product,     idcaja,     uniqueid, (select image from product where id_product=sales_detail_temp.id_product) as imagen , uniqueidtrans, (select product_name from product where id_product=sales_detail_temp.id_product) as product_name    FROM     sales_detail_temp where estado=0 and idcaja=" . $_SESSION['idcaja'] . " ) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select quantity as SC_metric1,item_total as SC_metric2, " . $cmps_quebras1 . " from (SELECT     id_sales_detail,     id_sales,     unit_cost,     unit_value,     quantity,     discount,     item_total,     id_product,     idcaja,     uniqueid, (select image from product where id_product=sales_detail_temp.id_product) as imagen , uniqueidtrans, (select product_name from product where id_product=sales_detail_temp.id_product) as product_name    FROM     sales_detail_temp where estado=0 and idcaja=" . $_SESSION['idcaja'] . " ) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT     id_sales_detail,     id_sales,     unit_cost,     unit_value,     quantity,     discount,     item_total,     id_product,     idcaja,     uniqueid, (select image from product where id_product=sales_detail_temp.id_product) as imagen , uniqueidtrans, (select product_name from product where id_product=sales_detail_temp.id_product) as product_name    FROM     sales_detail_temp where estado=0 and idcaja=" . $_SESSION['idcaja'] . " ) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_temp_resumen']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'id_product', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "id_product") {
                  $this->Lookup->lookup_id_product_id_product($$Cada_dim,  $id_product);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_id_product($where_ok);
