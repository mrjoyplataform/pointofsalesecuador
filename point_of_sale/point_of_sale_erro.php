...
$old_error_handler = set_error_handler("point_of_sale_scriptcase_error_handler");

function point_of_sale_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['point_of_sale']['contr_erro']) && $_SESSION['scriptcase']['point_of_sale']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_point_of_sale->contr_point_of_sale->Apl_com_erro = true;
            if ($inicial_point_of_sale->contr_point_of_sale->NM_ajax_flag)
            {
                $inicial_point_of_sale->contr_point_of_sale->NM_ajax_info['errList']['geral_point_of_sale'][] = $msg;
                return;
            }
            point_of_sale_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function point_of_sale_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['point_of_sale'])  && '' != $_SESSION['scriptcase']['error_icon']['point_of_sale'])  ? $_SESSION['scriptcase']['error_icon']['point_of_sale']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['point_of_sale']) && '' != $_SESSION['scriptcase']['error_close']['point_of_sale']) ? $_SESSION['scriptcase']['error_close']['point_of_sale'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['point_of_sale']) && '' != $_SESSION['scriptcase']['error_span_title']['point_of_sale']) ? $_SESSION['scriptcase']['error_span_title']['point_of_sale'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['point_of_sale']) && '' != $_SESSION['scriptcase']['error_icon_title']['point_of_sale']) ? $_SESSION['scriptcase']['error_icon_title']['point_of_sale'] : '';
?>
