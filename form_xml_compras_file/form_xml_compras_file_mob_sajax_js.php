  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_xml_compras_file_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_xml_compras_file_mob_form();
	}
...
?>
  ...
  // ---------- Validate fileblob
  function do_ajax_form_xml_compras_file_mob_validate_fileblob()
  {
    var nomeCampo_fileblob = "fileblob";
    var var_fileblob = scAjaxGetFieldText(nomeCampo_fileblob);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_xml_compras_file_mob_validate_fileblob(var_fileblob, var_script_case_init, do_ajax_form_xml_compras_file_mob_validate_fileblob_cb);
  } // do_ajax_form_xml_compras_file_mob_validate_fileblob

  function do_ajax_form_xml_compras_file_mob_validate_fileblob_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "fileblob";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_xml_compras_file_mob_validate_fileblob_cb

  // ---------- Validate idfile
  function do_ajax_form_xml_compras_file_mob_validate_idfile()
  {
    var nomeCampo_idfile = "idfile";
    var var_idfile = scAjaxGetFieldHidden(nomeCampo_idfile);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_xml_compras_file_mob_validate_idfile(var_idfile, var_script_case_init, do_ajax_form_xml_compras_file_mob_validate_idfile_cb);
  } // do_ajax_form_xml_compras_file_mob_validate_idfile

  function do_ajax_form_xml_compras_file_mob_validate_idfile_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "idfile";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_xml_compras_file_mob_validate_idfile_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_form_xml_compras_file_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_xml_compras_file_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_fileblob = scAjaxGetFieldText("fileblob");
    var var_idfile = scAjaxGetFieldHidden("idfile");
    var var_fileblob_ul_name = scAjaxSpecCharProtect(document.F1.fileblob_ul_name.value);//.replace(/[+]/g, "__NM_PLUS__");
    var var_fileblob_ul_type = document.F1.fileblob_ul_type.value;
    var var_fileblob_limpa = document.F1.fileblob_limpa.checked ? "S" : "N";
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_xml_compras_file_mob_submit_form(var_fileblob, var_idfile, var_fileblob_ul_name, var_fileblob_ul_type, var_fileblob_limpa, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, do_ajax_form_xml_compras_file_mob_submit_form_cb);
  } // do_ajax_form_xml_compras_file_mob_submit_form

  function do_ajax_form_xml_compras_file_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("fileblob");
      scAjaxHideErrorDisplay("idfile");
      scLigEditLookupCall();
<?php
...
?>
    ...
    if (!scAjaxHasError())
    {
      ...
      if (document.F1.fileblob_limpa && document.F1.fileblob_limpa.checked)
      {
        var oImgTemp = {0: {"value" : ""}};
        scAjaxSetFieldDocument("fileblob", oImgTemp, "", "");
      }
      else if (document.getElementById("id_ajax_doc_fileblob") && "" != document.getElementById("id_ajax_doc_fileblob").innerHTML)
      {
        document.getElementById("id_ajax_doc_fileblob").style.display = "";
      }
    }
    scAjaxShowDebug();
    scAjaxSetDisplay();
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_form_xml_compras_file_mob_submit_form_cb_after_alert);
  } // do_ajax_form_xml_compras_file_mob_submit_form_cb
  function do_ajax_form_xml_compras_file_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_xml_compras_file_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_xml_compras_file_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_xml_compras_file_mob_navigate_form, function() {});
    } else {
      perform_ajax_form_xml_compras_file_mob_navigate_form();
    }
  }

  function perform_ajax_form_xml_compras_file_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxHideErrorDisplay("fileblob");
    scAjaxHideErrorDisplay("idfile");
    var var_idfile = document.F2.idfile.value;
    ...
    scAjaxProcOn();
    x_ajax_form_xml_compras_file_mob_navigate_form(var_idfile, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search,  var_nmgp_cond_fast_search,  var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_xml_compras_file_mob_navigate_form_cb);
  } // do_ajax_form_xml_compras_file_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_xml_compras_file_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_xml_compras_file_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.idfile.value = scAjaxGetKeyValue("idfile");
    scAjaxSetSummary();
    scAjaxSetNavpage();
    scAjaxShowDebug();
    scAjaxSetLabel(true);
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_xml_compras_file_mob_navigate_form_cb_after_alert);
  } // do_ajax_form_xml_compras_file_mob_navigate_form_cb
  function do_ajax_form_xml_compras_file_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_form_xml_compras_file_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_form_xml_compras_file_mob_navigate_form_cb_after_alert

  function sc_hide_form_xml_compras_file_mob_form()
  {
  ...
  ajax_field_list[0] = "fileblob";
  ajax_field_list[1] = "idfile";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "fileblob": {"label": "", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "idfile": {"label": "", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["fileblob"][1] = "fileblob";
  ajax_field_mult["idfile"][1] = "idfile";

  var ajax_field_id = {
    "fileblob": new Array("hidden_field_label_fileblob", "hidden_field_data_fileblob"),
    "idfile": new Array("hidden_field_label_idfile", "hidden_field_data_idfile")
  };
