...
class grid_devices_monitor_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_devices_monitor']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_devices_monitor']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_ESTADO_ONLINE($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['contr_total_geral'] = "OK";
   } 

   //-----  estado_online
   function quebra_estado_online_ESTADO_ONLINE($estado_online, $arg_sum_estado_online) 
   {
      global $tot_estado_online;
      $tot_estado_online = array() ;  
      $tot_estado_online[0] = $estado_online ; 
   }
   //-----  cod_grupo
   function quebra_cod_grupo_ESTADO_ONLINE($estado_online, $cod_grupo, $arg_sum_estado_online, $arg_sum_cod_grupo) 
   {
      global $tot_cod_grupo;
      $tot_cod_grupo = array() ;  
      $tot_cod_grupo[0] = $cod_grupo ; 
   }

   //----- 
   function Calc_resumo_ESTADO_ONLINE($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('estado_online' => "ESTADO_ONLINE",'cod_grupo' => "cod_grupo");
      $cmps_quebra_atual = array("estado_online","cod_grupo");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_ESTADO_ONLINE($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_ESTADO_ONLINE($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('cod_device' => 'N','activa' => 'N','estado' => 'N','valor_default' => 'N','acepta_tiempo' => 'N','acepta_tokens' => 'N','tipo_dev' => 'N','direccion_torno' => 'N','timeout_rfid' => 'N','discapacitado' => 'N','numcaja' => 'N','empleado' => 'N','tokens' => 'N','bidireccion' => 'N','cod_devicee' => 'N','segundos' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_devices_monitor']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['estado_online']['reg'] = "S";
      $format_dimensions['estado_online']['msk'] = "";
      $format_dimensions['cod_grupo']['reg'] = "S";
      $format_dimensions['cod_grupo']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'ESTADO_ONLINE', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "estado_online") {
              }
              if ($Cada_dim == "cod_grupo") {
                  $this->Lookup->lookup_ESTADO_ONLINE_cod_grupo($$Cada_dim,  $cod_grupo);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_ESTADO_ONLINE($where_ok);
