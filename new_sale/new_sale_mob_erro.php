...
$old_error_handler = set_error_handler("new_sale_mob_scriptcase_error_handler");

function new_sale_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['new_sale_mob']['contr_erro']) && $_SESSION['scriptcase']['new_sale_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_new_sale_mob->contr_new_sale_mob->Apl_com_erro = true;
            if ($inicial_new_sale_mob->contr_new_sale_mob->NM_ajax_flag)
            {
                $inicial_new_sale_mob->contr_new_sale_mob->NM_ajax_info['errList']['geral_new_sale_mob'][] = $msg;
                return;
            }
            new_sale_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function new_sale_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['new_sale_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['new_sale_mob'])  ? $_SESSION['scriptcase']['error_icon']['new_sale_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['new_sale_mob']) && '' != $_SESSION['scriptcase']['error_close']['new_sale_mob']) ? $_SESSION['scriptcase']['error_close']['new_sale_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['new_sale_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['new_sale_mob']) ? $_SESSION['scriptcase']['error_span_title']['new_sale_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['new_sale_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['new_sale_mob']) ? $_SESSION['scriptcase']['error_icon_title']['new_sale_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['new_sale']['error_buffer'] .= ob_get_flush();
}
