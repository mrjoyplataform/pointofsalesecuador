  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_upload_txt_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_upload_txt_mob_form();
	}
...
?>
  ...
  // ---------- Validate txtfile
  function do_ajax_control_upload_txt_mob_validate_txtfile()
  {
    var nomeCampo_txtfile = "txtfile";
    var var_txtfile = scAjaxGetFieldText(nomeCampo_txtfile);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_upload_txt_mob_validate_txtfile(var_txtfile, var_script_case_init, do_ajax_control_upload_txt_mob_validate_txtfile_cb);
  } // do_ajax_control_upload_txt_mob_validate_txtfile

  function do_ajax_control_upload_txt_mob_validate_txtfile_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_upload_txt_mob_validate_txtfile_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_upload_txt_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_upload_txt_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_txtfile = scAjaxGetFieldText("txtfile");
    var var_txtfile_ul_name = scAjaxSpecCharProtect(document.F1.txtfile_ul_name.value);//.replace(/[+]/g, "__NM_PLUS__");
    var var_txtfile_ul_type = document.F1.txtfile_ul_type.value;
    var var_txtfile_salva = scAjaxSpecCharProtect(document.F1.txtfile_salva.value);//.replace(/[+]/g, "__NM_PLUS__");
    var var_txtfile_limpa = document.F1.txtfile_limpa.checked ? "S" : "N";
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn();
    x_ajax_control_upload_txt_mob_submit_form(var_txtfile, var_txtfile_ul_name, var_txtfile_ul_type, var_txtfile_salva, var_txtfile_limpa, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, do_ajax_control_upload_txt_mob_submit_form_cb);
  } // do_ajax_control_upload_txt_mob_submit_form

  function do_ajax_control_upload_txt_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    if (!scAjaxHasError())
    {
      ...
      if (document.F1.txtfile_limpa && document.F1.txtfile_limpa.checked)
      {
        var oImgTemp = {0: {"value" : ""}};
        scAjaxSetFieldDocument("txtfile", oImgTemp, "", "");
      }
      else if (document.getElementById("id_ajax_doc_txtfile") && "" != document.getElementById("id_ajax_doc_txtfile").innerHTML)
      {
        document.getElementById("id_ajax_doc_txtfile").style.display = "";
      }
    }
    scAjaxShowDebug();
    scAjaxSetDisplay();
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_upload_txt_mob_submit_form_cb_after_alert);
  } // do_ajax_control_upload_txt_mob_submit_form_cb
  function do_ajax_control_upload_txt_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_upload_txt_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_upload_txt_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_control_upload_txt_mob_navigate_form, function() {});
    } else {
      perform_ajax_control_upload_txt_mob_navigate_form();
    }
  }

  function perform_ajax_control_upload_txt_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_upload_txt_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_upload_txt_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_upload_txt_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_upload_txt_mob_navigate_form_cb_after_alert);
  } // do_ajax_control_upload_txt_mob_navigate_form_cb
  function do_ajax_control_upload_txt_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_upload_txt_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_upload_txt_mob_navigate_form_cb_after_alert

  function sc_hide_control_upload_txt_mob_form()
  {
  ...
  ajax_field_list[0] = "txtfile";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "txtfile": {"label": "", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["txtfile"][1] = "txtfile";

  var ajax_field_id = {
    "txtfile": new Array("hidden_field_label_txtfile", "hidden_field_data_txtfile")
  };
