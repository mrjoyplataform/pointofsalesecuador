...
$old_error_handler = set_error_handler("control_upload_txt_scriptcase_error_handler");

function control_upload_txt_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['control_upload_txt']['contr_erro']) && $_SESSION['scriptcase']['control_upload_txt']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_upload_txt->contr_control_upload_txt->Apl_com_erro = true;
            if ($inicial_control_upload_txt->contr_control_upload_txt->NM_ajax_flag)
            {
                $inicial_control_upload_txt->contr_control_upload_txt->NM_ajax_info['errList']['geral_control_upload_txt'][] = $msg;
                return;
            }
            control_upload_txt_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function control_upload_txt_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_upload_txt'])  && '' != $_SESSION['scriptcase']['error_icon']['control_upload_txt'])  ? $_SESSION['scriptcase']['error_icon']['control_upload_txt']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_upload_txt']) && '' != $_SESSION['scriptcase']['error_close']['control_upload_txt']) ? $_SESSION['scriptcase']['error_close']['control_upload_txt'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_upload_txt']) && '' != $_SESSION['scriptcase']['error_span_title']['control_upload_txt']) ? $_SESSION['scriptcase']['error_span_title']['control_upload_txt'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_upload_txt']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_upload_txt']) ? $_SESSION['scriptcase']['error_icon_title']['control_upload_txt'] : '';
?>
