...
$old_error_handler = set_error_handler("control_sinsaldo_tokens_scriptcase_error_handler");

function control_sinsaldo_tokens_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['control_sinsaldo_tokens']['contr_erro']) && $_SESSION['scriptcase']['control_sinsaldo_tokens']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_sinsaldo_tokens->contr_control_sinsaldo_tokens->Apl_com_erro = true;
            if ($inicial_control_sinsaldo_tokens->contr_control_sinsaldo_tokens->NM_ajax_flag)
            {
                $inicial_control_sinsaldo_tokens->contr_control_sinsaldo_tokens->NM_ajax_info['errList']['geral_control_sinsaldo_tokens'][] = $msg;
                return;
            }
            control_sinsaldo_tokens_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function control_sinsaldo_tokens_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_sinsaldo_tokens'])  && '' != $_SESSION['scriptcase']['error_icon']['control_sinsaldo_tokens'])  ? $_SESSION['scriptcase']['error_icon']['control_sinsaldo_tokens']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_sinsaldo_tokens']) && '' != $_SESSION['scriptcase']['error_close']['control_sinsaldo_tokens']) ? $_SESSION['scriptcase']['error_close']['control_sinsaldo_tokens'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_sinsaldo_tokens']) && '' != $_SESSION['scriptcase']['error_span_title']['control_sinsaldo_tokens']) ? $_SESSION['scriptcase']['error_span_title']['control_sinsaldo_tokens'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_sinsaldo_tokens']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_sinsaldo_tokens']) ? $_SESSION['scriptcase']['error_icon_title']['control_sinsaldo_tokens'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['control_sinsaldo_tokens']['error_buffer'] .= ob_get_flush();
}
