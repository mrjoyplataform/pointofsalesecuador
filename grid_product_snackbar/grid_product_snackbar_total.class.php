...
class grid_product_snackbar_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_product_snackbar']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_product_snackbar']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_id_category($res_limit=false)
   {
      global $nada, $nm_lang , $id_status;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['contr_total_geral'] = "OK";
   } 

   //-----  id_status
   function quebra_id_status_id_category($id_status, $arg_sum_id_status) 
   {
      global $tot_id_status, $id_status;
      $tot_id_status = array() ;  
      $tot_id_status[0] = $id_status ; 
   }
   //-----  id_category
   function quebra_id_category_id_category($id_status, $id_category, $arg_sum_id_status, $arg_sum_id_category) 
   {
      global $tot_id_category, $id_status;
      $tot_id_category = array() ;  
      $tot_id_category[0] = $id_category ; 
   }

   //----- 
   function Calc_resumo_id_category($destino_resumo)
   {
      global $nm_lang, $id_status;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('id_status' => "id_status",'id_category' => "id_category");
      $cmps_quebra_atual = array("id_status","id_category");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_id_category($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_id_category($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('id_product' => 'N','id_status' => 'N','product_value' => 'N','product_cost' => 'N','discount' => 'N','stock' => 'N','id_category' => 'N','service' => 'N','kiosko' => 'N','entrada' => 'N','jugador' => 'N','visitante' => 'N','tarjeta' => 'N','minutos' => 'N','tokens' => 'N','comida' => 'N','score' => 'N','poriva' => 'N','precioventa' => 'N','puntoventa' => 'N','orden' => 'N','arcade_clasic' => 'N','ingrediente' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_product_snackbar']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['id_status']['reg'] = "S";
      $format_dimensions['id_status']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'id_category', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "id_status") {
                  $this->Lookup->lookup_id_category_id_status($$Cada_dim,  $id_status);
              }
              if ($Cada_dim == "id_category") {
                  $this->Lookup->lookup_id_category_id_category($$Cada_dim,  $id_category);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_id_category($where_ok);
