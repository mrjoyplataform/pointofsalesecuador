...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_pago']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_pago"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_pagarbtn = false;
function sc_btn_pagarbtn()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_pagarbtn(); }, 500);
      return;
    }
    if (Crtl_btn_pagarbtn) {return;}
    sc_btn_pagarbtn_ok();
}
function sc_btn_pagarbtn_cancel()
{
}
function sc_btn_pagarbtn_ok()
{
    Crtl_btn_pagarbtn = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?pagarbtn?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var Crtl_btn_volver = false;
...
{
    Crtl_btn_volver = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?volver?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_pago']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_control_pago_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_control_pago_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "control_pago_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "control_pago_fim.php";
    document.F4.submit();
  }
}
