...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_telefono_registro']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_telefono_registro"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_Saltar = false;
function sc_btn_Saltar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Saltar(); }, 500);
      return;
    }
    if (Crtl_btn_Saltar) {return;}
    sc_btn_Saltar_ok();
}
function sc_btn_Saltar_cancel()
{
}
function sc_btn_Saltar_ok()
{
    Crtl_btn_Saltar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Saltar?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_telefono_registro']['where_detal']))
  {
?>  
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "control_telefono_registro_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "control_telefono_registro_fim.php";
    document.F4.submit();
  }
}
