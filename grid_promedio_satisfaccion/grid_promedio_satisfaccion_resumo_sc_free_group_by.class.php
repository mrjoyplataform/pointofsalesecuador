...
class grid_promedio_satisfaccion_resumo
{
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['arr_total']['fecha'] = $this->array_total_fecha;
   ...
   function completeMatrix()
   {
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['labels']['fecha'] = $Lab_fecha; 
       ...
       //-----
       $this->comp_sum = array(
           2 => "Cantidad (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_avg'] . ")",
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
      ...
      eval ($Str_gb);
       ...
       $Arr_parms['idproducto']['order_datatype'] = 'tinyint';
       //-----
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
       ...
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['pivot_order_level'] = 0;
       }
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           'fecha' => array(
           '1' => array(
               'title'    => $this->getChartText(strip_tags("Cantidad (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_avg'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'fecha',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_fecha)),
               'label_y'  => $this->getChartText(strip_tags("Cantidad (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_avg'] . ")")),
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
...
{ 
   ...
   $nm_saida->saida("          nm_submit_modal(\"" . $this->Ini->path_link . "grid_promedio_satisfaccion/grid_promedio_satisfaccion_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=pdf_res&sAdd=__E__nmgp_tipo_pdf=\" + z + \"__E__sc_parms_pdf=\" + p + \"__E__sc_create_charts=\" + crt + \"__E__sc_graf_pdf=\" + g + \"__E__chart_level=\" + chart_level + \"__E__SC_module_export=\" + SC_module_export + \"__E__use_pass_pdf=\" + use_pass_pdf + \"__E__pdf_all_cab=\" + pdf_all_cab + \"__E__pdf_all_label=\" +  pdf_all_label + \"__E__pdf_label_group=\" +  pdf_label_group + \"__E__pdf_zip=\" +  pdf_zip + \"&nm_opc=pdf&KeepThis=true&TB_iframe=true&modal=true\", '');\r\n");
      ...
      $nm_saida->saida(" }\r\n");
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['SC_Gb_Free_cmp']))
      {
...
//--- 
   ...
   //---- 
   function adiciona_registro($avg_cantidad, $avg_promedio_califica, $quebra_fecha, $quebra_idproducto, $quebra_fecha_orig, $quebra_idproducto_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['grid_promedio_satisfaccion']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
          ...
          $cmp_look   = "Cmp_formt";
          $cmp_orig   = "Cmp_arg";
          if (empty($avg_cantidad)) {$avg_cantidad = '0';}
          if (empty($avg_promedio_califica)) {$avg_promedio_califica = '0';}
          eval ('
          if (!isset($this->' . $arr_name . '))
          {
              $this->' . $arr_name . '[0] = 1;
              $this->' . $arr_name . '[1] = ' . $avg_cantidad . ';
              $this->' . $arr_name . '[2] = ' . $avg_promedio_califica . ';
          ...
          {
              $this->' . $arr_name . '[0]++;
              $this->' . $arr_name . '[1] = bcadd($this->' . $arr_name . '[1], ' . $avg_cantidad . ', 10);
              $this->' . $arr_name . '[2] = bcadd($this->' . $arr_name . '[2], ' . $avg_promedio_califica . ', 10);
          }
          ');
      }
   }
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->cantidad, $this->promedio_califica, $this->fecha, $this->idproducto, $this->fecha_orig,  $this->idproducto_orig);
            }
   }
//
