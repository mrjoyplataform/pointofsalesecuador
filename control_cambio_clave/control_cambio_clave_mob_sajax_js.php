  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_cambio_clave_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_cambio_clave_mob_form();
	}
...
?>
  ...
  // ---------- Validate pin
  function do_ajax_control_cambio_clave_mob_validate_pin()
  {
    var nomeCampo_pin = "pin";
    var var_pin = scAjaxGetFieldText(nomeCampo_pin);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cambio_clave_mob_validate_pin(var_pin, var_script_case_init, do_ajax_control_cambio_clave_mob_validate_pin_cb);
  } // do_ajax_control_cambio_clave_mob_validate_pin

  function do_ajax_control_cambio_clave_mob_validate_pin_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cambio_clave_mob_validate_pin_cb

  // ---------- Validate pinvalida
  function do_ajax_control_cambio_clave_mob_validate_pinvalida()
  {
    var nomeCampo_pinvalida = "pinvalida";
    var var_pinvalida = scAjaxGetFieldText(nomeCampo_pinvalida);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cambio_clave_mob_validate_pinvalida(var_pinvalida, var_script_case_init, do_ajax_control_cambio_clave_mob_validate_pinvalida_cb);
  } // do_ajax_control_cambio_clave_mob_validate_pinvalida

  function do_ajax_control_cambio_clave_mob_validate_pinvalida_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "pinvalida";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sAppErrors = scAjaxListErrors(true);
    if ("" == sAppErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cambio_clave_mob_validate_pinvalida_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_cambio_clave_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_cambio_clave_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_pin = scAjaxGetFieldText("pin");
    var var_pinvalida = scAjaxGetFieldText("pinvalida");
    ...
    scAjaxProcOn();
    x_ajax_control_cambio_clave_mob_submit_form(var_pin, var_pinvalida, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_cambio_clave_mob_submit_form_cb);
  } // do_ajax_control_cambio_clave_mob_submit_form

  function do_ajax_control_cambio_clave_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_cambio_clave_mob_submit_form_cb_after_alert);
  } // do_ajax_control_cambio_clave_mob_submit_form_cb
  function do_ajax_control_cambio_clave_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_cambio_clave_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_cambio_clave_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_control_cambio_clave_mob_navigate_form, function() {});
    } else {
      perform_ajax_control_cambio_clave_mob_navigate_form();
    }
  }

  function perform_ajax_control_cambio_clave_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_cambio_clave_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_cambio_clave_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_cambio_clave_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_cambio_clave_mob_navigate_form_cb_after_alert);
  } // do_ajax_control_cambio_clave_mob_navigate_form_cb
  function do_ajax_control_cambio_clave_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scFocusField('pin');
...
{
?>
    do_ajax_control_cambio_clave_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_cambio_clave_mob_navigate_form_cb_after_alert

  function sc_hide_control_cambio_clave_mob_form()
  {
  ...
  ajax_field_list[0] = "pin";
  ajax_field_list[1] = "pinvalida";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "pin": {"label": "INGRESE NUEVO PIN", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "pinvalida": {"label": "REPITA NUEVO PIN", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["pinvalida"][1] = "pinvalida";

  var ajax_field_id = {
    "pin": new Array("hidden_field_label_pin", "hidden_field_data_pin"),
    "pinvalida": new Array("hidden_field_label_pinvalida", "hidden_field_data_pinvalida")
  };
