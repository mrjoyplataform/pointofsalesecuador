...
class grid_customer_clubmrjoy_process_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_customer_clubmrjoy_process']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_customer_clubmrjoy_process']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from (SELECT      idreg,     idcustomer,     nombre,     correo,     celular,     idinterno,     direccion,     tipodoc,     ult_fecha,     local_centro,     cliente_tipo,     afiliado_desde,     sector,     estado,     uniqueid,     ciudad,     pais,     fecha1,     fecha2,     fecha3 FROM      customer_clubmrjoy where idcustomer not in (select idcustomer from customer_clubmrjoy_matrix)) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*) from (SELECT      idreg,     idcustomer,     nombre,     correo,     celular,     idinterno,     direccion,     tipodoc,     ult_fecha,     local_centro,     cliente_tipo,     afiliado_desde,     sector,     estado,     uniqueid,     ciudad,     pais,     fecha1,     fecha2,     fecha3 FROM      customer_clubmrjoy where idcustomer not in (select idcustomer from customer_clubmrjoy_matrix)) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['contr_total_geral'] = "OK";
   } 

   //-----  local_centro
   function quebra_local_centro_sc_free_group_by($local_centro, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_local_centro = array() ;  
      $tot_local_centro[0] = $local_centro ; 
   }
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('local_centro' => "local_centro");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('idreg' => 'N','estado' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*)#@#cmps_quebras#@# from (SELECT      idreg,     idcustomer,     nombre,     correo,     celular,     idinterno,     direccion,     tipodoc,     ult_fecha,     local_centro,     cliente_tipo,     afiliado_desde,     sector,     estado,     uniqueid,     ciudad,     pais,     fecha1,     fecha2,     fecha3 FROM      customer_clubmrjoy where idcustomer not in (select idcustomer from customer_clubmrjoy_matrix)) sc_sel_esp " . $where_ok;
         $comando  = "select count(*)#@#cmps_quebras#@# from (";
         $comando .= "select " . $cmps_quebras1 . " from (SELECT      idreg,     idcustomer,     nombre,     correo,     celular,     idinterno,     direccion,     tipodoc,     ult_fecha,     local_centro,     cliente_tipo,     afiliado_desde,     sector,     estado,     uniqueid,     ciudad,     pais,     fecha1,     fecha2,     fecha3 FROM      customer_clubmrjoy where idcustomer not in (select idcustomer from customer_clubmrjoy_matrix)) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT      idreg,     idcustomer,     nombre,     correo,     celular,     idinterno,     direccion,     tipodoc,     ult_fecha,     local_centro,     cliente_tipo,     afiliado_desde,     sector,     estado,     uniqueid,     ciudad,     pais,     fecha1,     fecha2,     fecha3 FROM      customer_clubmrjoy where idcustomer not in (select idcustomer from customer_clubmrjoy_matrix)) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_customer_clubmrjoy_process']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['local_centro']['reg'] = "S";
      $format_dimensions['local_centro']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
