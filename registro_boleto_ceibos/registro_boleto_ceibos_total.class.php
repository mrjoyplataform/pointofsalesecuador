...
class registro_boleto_ceibos_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['registro_boleto_ceibos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['registro_boleto_ceibos']['campos_busca']))
      { 
   ...
   function quebra_geral_sc_free_total($res_limit=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(puntos) from (SELECT     qrboleto,     codrfid,     fecha_registro,     uidtrans,      idsale,     idproducto,     ultdevice,     ultfecha,     estado,     nombre,     fecha_nacimiento,     genero,     username,     fecha_activacion,     fecha_entrada,     fecha_salida,     producto,     entrada,     jugador,     visitante,       minutos,      puntos ,  (date_format( now(), '%Y' ) - date_format( fecha_nacimiento, '%Y' ) -   ( date_format( now(), '00-%m-%d') < date_format( fecha_nacimiento, '00-%m-%d' ) )) as edad  , idchar , registro ,   (select picture_char from charplayers where id_char=idchar) as characterpic , cliente, idcliente, telefono, cliente_email , serie       FROM     view_boletos_sales where (qrboleto='" . $_SESSION['boletoqr'] . "'    ) ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(puntos) from (SELECT     qrboleto,     codrfid,     fecha_registro,     uidtrans,      idsale,     idproducto,     ultdevice,     ultfecha,     estado,     nombre,     fecha_nacimiento,     genero,     username,     fecha_activacion,     fecha_entrada,     fecha_salida,     producto,     entrada,     jugador,     visitante,       minutos,      puntos ,  (date_format( now(), '%Y' ) - date_format( fecha_nacimiento, '%Y' ) -   ( date_format( now(), '00-%m-%d') < date_format( fecha_nacimiento, '00-%m-%d' ) )) as edad  , idchar , registro ,   (select picture_char from charplayers where id_char=idchar) as characterpic , cliente, idcliente, telefono, cliente_email , serie       FROM     view_boletos_sales where (qrboleto='" . $_SESSION['boletoqr'] . "'    ) ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['registro_boleto_ceibos']['contr_total_geral'] = "OK";
   } 
