...
class grid_xml_compras_hdr_pendientes_todas_xls
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_xml_compras_hdr_pendientes_todas/grid_xml_compras_hdr_pendientes_todas_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              ...
              $this->Res_xls = new grid_xml_compras_hdr_pendientes_todas_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_xml_compras_hdr_pendientes_todas.zip";
          $this->Arquivo   .= "_grid_xml_compras_hdr_pendientes_todas" . $this->Xls_tp;
          $this->Tit_doc    = "grid_xml_compras_hdr_pendientes_todas" . $this->Xls_tp;
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name'] .= $this->Xls_tp;
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
      ...
      while (!$rs->EOF)
      {
         ...
         $this->arg_sum_identificacioncomprador = " = " . $this->Db->qstr($this->identificacioncomprador);
          ...
          {  
              ...
              if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida'] && !$prim_gb && $this->groupby_show == "S") {
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   function NM_export_idfile()
   {
         ...
         if (is_numeric($this->idfile))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->idfile);
         $this->Xls_col++;
   }
   //----- estado
   ...
   //----- idfile
   function NM_sub_cons_idfile()
   {
         $this->idfile = NM_charset_to_utf8($this->idfile);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->idfile;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "right";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['nolabel'])
       {
 ...
 function quebra_tipodoc_identificacionComprador($identificacioncomprador, $tipodoc) 
 {
   ...
   function quebra_identificacioncomprador_identificacionComprador_bot()
   {
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['field_order'] as $Cada_cmp)
       {
           ...
           elseif (!isset($this->NM_cmp_hidden[$Cada_cmp]) || $this->NM_cmp_hidden[$Cada_cmp] != "off")
           {
               ...
               elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida']) {
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = "";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
   ...
   function quebra_geral_identificacionComprador_bot()
   {
       ...
       $prim_cmp = true;
       $mens_tot = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['tot_geral'][0] . "(" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['tot_geral'][1] . ")";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['field_order'] as $Cada_cmp)
       {
           if ($Cada_cmp == "totalsinimpuestos" && (!isset($this->NM_cmp_hidden['totalsinimpuestos']) || $this->NM_cmp_hidden['totalsinimpuestos'] != "off"))
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['tot_geral'][2];
               $prim_cmp = false;
           ...
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['tot_geral'][3];
               $prim_cmp = false;
               $Vl_Tot = NM_charset_to_utf8($Vl_Tot);
               if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['embutida']) {
                   $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $Vl_Tot;
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_file'] = $this->Xls_f;
      }
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_xml_compras_hdr_pendientes_todas']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
