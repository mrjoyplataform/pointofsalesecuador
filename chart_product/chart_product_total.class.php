...
class chart_product_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_product']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_product']['campos_busca']))
      { 
   ...
   function quebra_geral_sc_free_group_by($res_limit=false)
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_fechatrans_sc_free_group_by($fechatrans, $Where_qb, $Cmp_Name) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['where_pesq'])) 
      { 
   ...
   //-----  id_product
   function quebra_id_product_sc_free_group_by($id_product, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      ...
      $tot_id_product[0] = NM_encode_input(sc_strip_script($id_product)) ; 
      ...
      $rt->Close(); 
      $$Var_name_gb = $tot_id_product;
   } 

   //-----  fechatrans_scgb_125
   function quebra_fechatrans_scgb_125_sc_free_group_by($fechatrans_scgb_125, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_fechatrans_scgb_125 = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(quantity), sum(item_total) from (SELECT     id_sales_detail,     id_sales,     unit_cost,     unit_value,     quantity,     discount,     item_total,     id_product,     idcaja,     uniqueid,     uniqueidtrans,     fechatrans,     estado,     valor_siniva,     valor_unitario,     codproducto,     factura,     producto,     entrada,     jugador,     visitante,     tarjeta,     tokens,     minutos,     comida,     tarjetarfid,     rfid_pulsera,(select cuentaventa from product where product.id_product=sales_detail.id_product) as cuentaventa FROM     sales_detail) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['where_pesq']; 
      } 
      ...
      $tot_fechatrans_scgb_125[0] = NM_encode_input(sc_strip_script($fechatrans_scgb_125)) ; 
      $tot_fechatrans_scgb_125[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fechatrans_scgb_125[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fechatrans_scgb_125[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_fechatrans_scgb_125;
   } 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fechatrans' => "fechatrans",'cuentaventa' => "cuentaventa",'id_product' => "id_product",'fechatrans_scgb_125' => "fechatrans");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_product']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['fechatrans']['reg'] = "N";
      $format_dimensions['fechatrans']['msk'] = "F/Y";
      ...
      $format_dimensions['fechatrans_scgb_125']['reg'] = "S";
      $format_dimensions['fechatrans_scgb_125']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "id_product") {
                  $this->Lookup->lookup_sc_free_group_by_id_product($$Cada_dim,  $id_product);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
