  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_controlrfid_mob" == sTestField)
        {
...
?>
  ...
  // ---------- Validate cod_rfid
  function do_ajax_controlrfid_mob_validate_cod_rfid()
  {
    var nomeCampo_cod_rfid = "cod_rfid";
    var var_cod_rfid = scAjaxGetFieldText(nomeCampo_cod_rfid);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_controlrfid_mob_validate_cod_rfid(var_cod_rfid, var_script_case_init, do_ajax_controlrfid_mob_validate_cod_rfid_cb);
  } // do_ajax_controlrfid_mob_validate_cod_rfid

  function do_ajax_controlrfid_mob_validate_cod_rfid_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "cod_rfid";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_controlrfid_mob_validate_cod_rfid_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_controlrfid_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_controlrfid_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_cod_rfid = scAjaxGetFieldText("cod_rfid");
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn();
    x_ajax_controlrfid_mob_submit_form(var_cod_rfid, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, do_ajax_controlrfid_mob_submit_form_cb);
  } // do_ajax_controlrfid_mob_submit_form

  function do_ajax_controlrfid_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_controlrfid_mob_submit_form_cb_after_alert);
  } // do_ajax_controlrfid_mob_submit_form_cb
  function do_ajax_controlrfid_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_controlrfid_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_controlrfid_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_controlrfid_mob_navigate_form, function() {});
    } else {
      perform_ajax_controlrfid_mob_navigate_form();
    }
  }

  function perform_ajax_controlrfid_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_controlrfid_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['controlrfid_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_controlrfid_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_controlrfid_mob_navigate_form_cb_after_alert);
  } // do_ajax_controlrfid_mob_navigate_form_cb
  function do_ajax_controlrfid_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_controlrfid_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_controlrfid_mob_navigate_form_cb_after_alert

  function sc_hide_controlrfid_mob_form()
  {
  ...
  ajax_field_list[0] = "cod_rfid";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "cod_rfid": {"label": "RFID", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["cod_rfid"][1] = "cod_rfid";

  var ajax_field_id = {
    "cod_rfid": new Array("hidden_field_label_cod_rfid", "hidden_field_data_cod_rfid")
  };
