...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_read_tarjeta_rfid_decimal_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_read_tarjeta_rfid_decimal_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['control_read_tarjeta_rfid_decimal_mob']['embutida_form']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
if ($_SESSION['sc_session'][$this->Ini->sc_page]['control_read_tarjeta_rfid_decimal_mob']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['control_read_tarjeta_rfid_decimal_mob']['run_iframe'] == "R") {
    echo "   sob_iframe += 'parent.';\r\n";
}
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_read_tarjeta_rfid_decimal_mob']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['control_read_tarjeta_rfid_decimal_mob']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_read_tarjeta_rfid_decimal_mob']['where_detal']))
  {
?>  
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "control_read_tarjeta_rfid_decimal_mob_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "control_read_tarjeta_rfid_decimal_mob_fim.php";
    document.F4.submit();
  }
}
