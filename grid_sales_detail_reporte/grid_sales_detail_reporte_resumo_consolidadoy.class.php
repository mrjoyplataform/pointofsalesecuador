...
class grid_sales_detail_reporte_resumo
{
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['tot_geral'][2];
      $this->array_total_geral[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['tot_geral'][3];
   ...
   function completeMatrix()
   {
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_order']['consolidadoy']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_order']['consolidadoy'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_order']['consolidadoy'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_display']['consolidadoy'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_display']['consolidadoy'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_display']['consolidadoy'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_display']['consolidadoy'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_display']['consolidadoy'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_display']['consolidadoy'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['summarizing_fields_display']['consolidadoy'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_cuentaventa as $i_cuentaventa => $d_cuentaventa) {
           if (!in_array($i_cuentaventa, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_cuentaventa[4] ] = $d_cuentaventa[3];
               $this->comp_label[0][ $d_cuentaventa[4] ] = $d_cuentaventa[3];
           }
           foreach ($this->array_total_producto[$i_cuentaventa] as $i_producto => $d_producto) {
               if (!in_array($i_producto, $this->comp_label[1][ $d_cuentaventa[4] ], true)) {
                   $this->comp_index[1][ $d_producto[4] ] = $d_producto[3];
                   $this->comp_label[1][ $d_cuentaventa[4] ][ $d_producto[4] ] = $d_producto[3];
               }
           }
       }
       ...
       //-----
       foreach ($this->comp_label[0] as $i_cuentaventa => $l_cuentaventa) {
           if (isset($this->array_total_cuentaventa[$i_cuentaventa])) {
               $this->comp_group[$i_cuentaventa] = array();
           }
           foreach ($this->comp_label[1][$i_cuentaventa] as $i_producto => $l_producto) {
               if (isset($this->array_total_producto[$i_cuentaventa][$i_producto])) {
                   $this->comp_group[$i_cuentaventa][$i_producto] = array();
               }
           }
       }
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sales_detail_reporte']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_consolidadoy("res");
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_quantity, $sum_item_total, $quebra_cuentaventa, $quebra_producto, $quebra_cuentaventa_orig, $quebra_producto_orig)
   {
      //----- Cuentaventa
      if (!isset($this->array_total_cuentaventa[$quebra_cuentaventa_orig]))
      {
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][0] = 1;
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][1] = $sum_quantity;
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][2] = $sum_item_total;
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][3] = $quebra_cuentaventa;
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][4] = $quebra_cuentaventa_orig;
      }
      else
      {
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][0]++;
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][1] = bcadd($this->array_total_cuentaventa[$quebra_cuentaventa_orig][1], $sum_quantity, 10);
         $this->array_total_cuentaventa[$quebra_cuentaventa_orig][2] = bcadd($this->array_total_cuentaventa[$quebra_cuentaventa_orig][2], $sum_item_total, 10);
      }
      //----- Producto
      if (!isset($this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig]))
      {
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][0] = 1;
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][1] = $sum_quantity;
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][2] = $sum_item_total;
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][3] = $quebra_producto;
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][4] = $quebra_producto_orig;
      }
      else
      {
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][0]++;
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][1] = bcadd($this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][1], $sum_quantity, 10);
         $this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][2] = bcadd($this->array_total_producto[$quebra_cuentaventa_orig][$quebra_producto_orig][2], $sum_item_total, 10);
      }
   }
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->quantity, $this->item_total, $this->cuentaventa, $this->producto, $this->cuentaventa_orig,  $this->producto_orig);
            }
   }
//
